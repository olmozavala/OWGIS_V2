...
owgis.ncwms.currents.particles.updateParticles = function updateParticles(i, j) {
	...
	// This case if when we do not have an animiation (single grid)
	if ((grids.length === 1) || (currentGrid === (grids.length - 1))) {
		...
		//If the lifetime of the particle has exceeded the limit we restart
	    ...
	    // Here is when we have an animation MULTIPLE GRID
...
//    console.log("Done initializing particle positions! (random)");
}