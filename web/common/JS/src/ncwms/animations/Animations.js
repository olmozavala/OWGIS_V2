...
function updateAnimationStatus(newStatus){
    ...
    if( owgis.ncwms.animation.status.current === owgis.ncwms.animation.status.loading && newStatus === owgis.ncwms.animation.status.playing){
        ...
        if( owgis.ncwms.animation.status.current === owgis.ncwms.animation.status.loadingplaying && newStatus === owgis.ncwms.animation.status.paused){
...
owgis.ncwms.animation.dispAnimation = function dispAnimation(){
    ...
    animLayer = new ol.layer.Image({source: animSource});
...
owgis.ncwms.animation.imageHasBeenLoadedParallel = function(e){
    ...
    if( owgis.ncwms.animation.status.current === owgis.ncwms.animation.status.loading || owgis.ncwms.animation.status.current === owgis.ncwms.animation.status.loadingplaying){
        var currentImage = parseInt(e.target.id);
...
function loopAnimation(){
    ...
    // Removing the :00:00.000Z from the text / cleaning the title date
