            ...
            */
            ...
            for(i=0; i<allFramesVP.length; i++){
                ...
                allDataVP[allFramesVP[i]]  = request.done(function(data, status, xhr) { /*console.log("done request for vp !!!",data.split("\n")[0]);*/ })
                                                    .fail(function(jqXHR, textStatus, errorThrown) { }); //maybe we should check if the response is 200 / 404 / 401 (?)
            }
            ...
            $( document ).ajaxStop(function() {
                ...
                if( typeof allDataVP[allFramesVP[vpCurrentFrame]] !== "undefined" && allDataVP[allFramesVP[vpCurrentFrame]].status == 200 ){
                    ...
                    if( !allDataVP[allFramesVP[vpCurrentFrame]].hasOwnProperty("responseXML") ){
                        ...
                        var curr_dates = new Date(allFramesVP[vpCurrentFrame]);
                        var letime = "";
                                
                        if(!_.isUndefined(curr_dates)){
                            letime =  (_curr_language == 'ES') ? "de "+curr_dates.getUTCHours() +":00, "+curr_dates.getUTCDate() +" de "+ meses[curr_dates.getUTCMonth()] : "from "+curr_dates.getUTCHours() +":00, "+curr_dates.getUTCDate() +" "+ meses[curr_dates.getUTCMonth()];
                        } else {
                            letime = "";
                        }     
                        ...
                        optionsChartVP = {
                            title: {
                                text: (_curr_language == "ES") ? "Perfil Vertical de "+titulo+", "+letime  : 'Vertical Profile of '+data.split('\n')[2].split(',')[1]+', Date '+letime
                            },
                            subtitle: {
                                text: latlon
                            },
                            ...
                            tooltip: {
                                pointFormat: "{point.y:.6f} "+ layerDetails.units
                            },
                            plotOptions: {
                                series: {
                                    marker: {
            ...
            function showButton(ajaxCan, latlon, first,data){
                    if(first == 1){
                        if(ajaxCan){
                            ...
                            var titulo1= (_curr_language == "ES") ? "Perfil Vertical de "+titulo+" "  : 'Vertical Profile of '+data.split('\n')[2].split(',')[1]+' ';
                            var popuplink = (mobile) ? "" : "<button id='newVerticalProfileWindow' onclick=\"openVertProf(allDataVP, allFramesVP, '"+latlon+"','"+titulo1+"');\" class='btn btn-default btn-xs' > <span class='glyphicon glyphicon-new-window' ></span> </button>";
                            ...
                            var dataLink = (_curr_language == "ES") ? "<b>Perfil Vertical: </b> <button type=\"button\" id=\"toAnimateVP\" class=\"btn btn-default btn-xs\" onclick=\"$('#showVertProf').modal('toggle');letsLoopVP(allDataVP, allFramesVP, '"+latlon+"');\">Mostrar</button><br>" : "<b>Vertical profile: </b> <button type=\"button\" id=\"toAnimateVP\" class=\"btn btn-default btn-xs\" onclick=\"$('#showVertProf').modal('toggle');letsLoopVP(allDataVP, allFramesVP, '"+latlon+"');\">Show</button><br>";
...
//
function openVertProf(allDataVP, allFramesVP, latlon, titulo){
    //open a new window with the highchart
    //var options = chart.userOptions,
    //container = chart.renderTo,
    var w,
        ...
        t = "Highcharts.chart(\"containerChartsVP\", " + JSON.stringify(optionsChartVP) + ");" +
                                            ...
                                            "function letsLoopVP(allDataVP,allFramesVP, latlon){"+ 
                                                ...
                                                "function animateVerticalProfile(){"+
                                                    ...
                                                    "Highcharts.chart('containerChartsVP', { title: { text: '"+titulo+"+', '+allFramesVP[vpCurrentFrame] },"+
                                                        " subtitle: {text: latlon}, chart: {type: 'spline', inverted: true}, yAxis: {lineWidth: 1}, xAxis: {title: {text: data_.split('\\n')[2].split(',')[0]},lineWidth: 1}, "+
                                                        " tooltip: { pointFormat: '{point.y:.6f}' + '"+layerDetails.units+"' }, "+
...
function letsLoopVP(allDataVP,allFramesVP, latlon){
    ...
    function animateVerticalProfile(){
        ...
        Highcharts.chart('containerChartsVP', {
                                title: {
                                  text: (_curr_language == 'ES') ? "Perfil vertical de "+titulo+", "+letime : 'Vertical Profile of '+data_.split('\n')[2].split(',')[1]+', Date '+letime
                                },
                                subtitle: {
                                  text: latlon
                                },
				...
				console.log(el_width, el_height);	
                                ...
                                if(!_.isUndefined(dateTexts) && !_.isUndefined(dateTexte)){
                                    if(!_.isUndefined(layerDetails.subtitleText)){
                                        if(layerDetails.subtitleText == "daily"){
                                            ...
                                            tdtlf= dtlf;
                                            letime =  (_curr_language == 'ES') ? "del "+curr_dates.getUTCHours() +":00, "+curr_dates.getUTCDate() +" de "+ meses[curr_dates.getUTCMonth()]+" al "+curr_datee.getUTCHours() +":00, "+curr_datee.getUTCDate() +" de "+ meses[curr_datee.getUTCMonth()] : "from "+curr_dates.getUTCHours() +":00, "+curr_dates.getUTCDate() +" "+ meses[curr_dates.getUTCMonth()]+" to "+curr_datee.getUTCHours() +":00, "+curr_datee.getUTCDate() +" "+ meses[curr_datee.getUTCMonth()];
                                        }
                                    } else {
                                        letime = ""; 
                                    }
				 ...
				 Highcharts.chart('containerChartsTS', {
					...
					height: el_height
                                    ...
                                    tooltip: {
                                        pointFormat: (layerDetails.subtitleText == "dailyxhour")? "{point.y:.6f} " + layerDetails.units : "{point.y:.2f} " + layerDetails.units,
